...
while getopts "hmpac:ri:s:" opt; do
  case ${opt} in
    h)
      help_msg
      ;;
...
if [ "$rosettafold" == "true" ]; then
	...
	while read line; do
		...
		if [ "$sizes" == "true" ]; then
			if [ "$L1" -lt "$min_size" ]; then
				echo "WARNING: $paired_name is smaller than $min_size"
				echo "WARNING: ignoring it"
				continue
			elif [ "$L2" -gt "$max_size" ]; then
				echo "WARNING: $paired_name is bigger than $max_size"
				echo "WARNING: ignoring it"
				continue
			fi
		fi
		...
		if [ "$is_homooligomer" == "true" ]; then
			...
			add_time "WARNING: Ignoring IDs line. Saved in ./to_remove.txt"
			echo "$input_a3m_file" >> to_remove.txt
...
if [ "$alphafold" == "true" ]; then
	...
	while read line; do
		...
		if [ "$AF2_conf_file" != "" ]; then
			...
			echo "Options: --num-models 5 --num-recycle 3 --rank iptm --stop-at-score 80 --recycle-early-stop-tolerance 1.5 --num-relax 1 --use-gpu-relax --save-all"
			colabfold_batch --num-models 5 --num-recycle 3 --rank iptm --stop-at-score 80 --recycle-early-stop-tolerance 1.5 --num-relax 1 --use-gpu-relax --save-all $input_a3m_file ./AF2/$paired_name
		fi
	done < "$IDs_table_file"
fi
